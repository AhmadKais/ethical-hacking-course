...
> - ☁️ **TryHackMe** (בדפדפן, ללא התקנה): [**Bash Scripting**](https://tryhackme.com/room/bashscripting) · "Python Basics"
> - 🐧 **Kali (מקומי):** כתוב והרץ את הסקריפטים מקומית ב-Kali (nano + python3/bash).

מקרא רמות: 🟢 קל · 🟡 בינוני · 🟠 מתקדם · 🔴 אתגר

---

## 🎮 האתגר הגדול — בנה נשק משלך

> כלים מוכנים זה נחמד — האקר אמיתי **כותב את שלו**. הפוך את ה-Port Scanner שתבנה ל**כלי recon אמיתי**: סרוק טווח פורטים, **תפוס Banner** (גרסת שירות) מכל פורט פתוח, ושמור לקובץ. עכשיו יש לך "nmap מיני" משלך.

  🏆 **תחרות:** מי הכלי המהיר ביותר שסורק 1000 פורטים? הוסף `threads` להאצה.
  ✅ הצלחה: יש לך כלי סריקה עובד שכתבת בעצמך — מבינים איך כלי אבטחה עובדים מבפנים.

> 💡 **עוד חידות באותו סגנון:** [PicoCTF](https://picoctf.org) (קטגוריית Programming) · [cmdchallenge.com](https://cmdchallenge.com).
