...
1. **DNS:** המחשב שואל שרת DNS "מה ה-IP של example.com?" ומקבל למשל `93.184.216.34`.
2. **ARP:** כדי לשלוח לרשת המקומית, המחשב מוצא את כתובת ה-MAC של ה-Gateway.
3. **TCP Handshake:** נפתח חיבור לפורט 80/443 (SYN → SYN-ACK → ACK).
4. **HTTP:** נשלחת בקשה `GET /`, והשרת מחזיר את הדף.

> 💡 כל שלב כאן שייך לשכבה אחרת במודל OSI — וכל שלב הוא גם **נקודת תקיפה אפשרית** (DNS Spoofing, ARP Poisoning, סריקת פורטים, חולשות HTTP).

---

## 3.2 מודל OSI

מודל **OSI** מתאר תקשורת ב-7 שכבות. כל שכבה מדברת עם המקבילה בצד השני, ומוסיפה/מסירה "עטיפה" משלה.
...
> 💡 **טריק לזכירה (7→1):** *All People Seem To Need Data Processing*.
...
- **IPv4:** 4 מספרים (0–255), למשל `192.168.1.10`. 32 סיביות. בפורמט עשרוני (Decimal).
...
- **MAC Address** — כתובת פיזית קבועה של כרטיס הרשת (שכבה 2), בפורמט `00:1A:2B:3C:4D:5E`. "צרובה" בחומרה — אך **ניתנת לזיוף** (MAC Spoofing).
- **ARP (Address Resolution Protocol)** — מתרגם כתובת IP לכתובת MAC ברשת המקומית. כשמכונה רוצה לשלוח ל-IP מקומי, היא "צועקת" ברשת: "מי מחזיק את ה-IP הזה?" והבעלים עונה עם ה-MAC שלו.

> 🔍 **ARP Poisoning (Man-in-the-Middle):** תוקף עונה לבקשות ARP בשקר, וגורם לתעבורה לעבור דרכו — כך הוא מאזין לכל התקשורת. זו מתקפת שכבה 2 קלאסית.

```bash
ip neigh      # טבלת ה-ARP (מיפוי IP↔MAC ידוע)
arp -a        # תחביר ישן
```
...
- **DNS (Domain Name System)** — מתרגם שם (`example.com`) ל-IP. עובד בעיקר ב-UDP פורט 53. יעד ל-OSINT ולתקיפות (DNS Spoofing, Zone Transfer).
- **DHCP** — מחלק כתובות IP אוטומטית למכשירים חדשים ברשת (יחד עם Gateway ו-DNS).
- **NAT (Network Address Translation)** — הראוטר "מתרגם" כתובות פרטיות רבות לכתובת ציבורית אחת. זו הסיבה שלכל המכשירים בבית יש `192.168.x.x` אך כתובת ציבורית אחת החוצה.
- **ICMP** — פרוטוקול בקרה; `ping` משתמש בו לבדיקת "חי/מת".

> 🔍 **Zone Transfer (`AXFR`)** — אם שרת DNS מוגדר שגוי, אפשר "לשאוב" ממנו את כל רשומות הדומיין — פרס ב-Recon:
> ```bash
